...
namespace ShopApi.Middlewares
{
    ...
    public class ExceptionMiddleware
    {
        ...
        {
            // Chi ap dung heuristics theo message cho cac loi "du kien" (custom exception).
            // Tranh viec map sai cac loi he thong/IO/Sql... thanh 404/401.
            var isAppException =
                ex is AppBadRequestException
                || ex is AppNotFoundException
                || ex is AppConflictException
                || ex is AppUnauthorizedException
                || ex is AppForbiddenException;
            ...
            if (isAppException && ex.Message.Equals("Unauthorized", StringComparison.OrdinalIgnoreCase))
                return HttpStatusCode.Unauthorized;

            if (isAppException && ex.Message.Contains("chua dang nhap", StringComparison.OrdinalIgnoreCase))
                return HttpStatusCode.Unauthorized;

            if (isAppException && ex.Message.Contains("not found", StringComparison.OrdinalIgnoreCase))
                return HttpStatusCode.NotFound;

            if (isAppException && ex.Message.Contains("khong tim thay", StringComparison.OrdinalIgnoreCase))
                return HttpStatusCode.NotFound;

            if (isAppException && (ex.Message.Contains("invalid", StringComparison.OrdinalIgnoreCase) ||
            ...
            if (isAppException && (ex.Message.Contains("khong hop le", StringComparison.OrdinalIgnoreCase) ||
